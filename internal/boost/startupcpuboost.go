...
import (
	...
	"github.com/google/kube-startup-cpu-boost/internal/dashboard"
...
const (
	...
	podEventUIDContainerName           = "__pod__"
)
...
func (b *StartupCPUBoostImpl) UpsertPod(ctx context.Context, pod *corev1.Pod) error {
	...
	if existing {
		...
		for _, event := range b.increaseEventsFromPod(pod) {
			dashboard.Report(ctx, event)
		}
	}
...
func (b *StartupCPUBoostImpl) increaseEventsFromPod(pod *corev1.Pod) []dashboard.Event {
	annotation, err := bpod.BoostAnnotationFromPod(pod)
	if err != nil {
		return nil
	}

	events := make([]dashboard.Event, 0, len(pod.Spec.Containers)+1)
	for _, container := range pod.Spec.Containers {
		requestBefore, hasRequest := annotation.InitCPURequests[container.Name]
		limitBefore, hasLimit := annotation.InitCPULimits[container.Name]
		if !hasRequest && !hasLimit {
			continue
		}
		ownerKind, ownerName := controllerOwner(pod)
		events = append(events, dashboard.Event{
			EventUID:          eventUID(dashboard.EventIncrease, pod, container.Name, annotation.BoostTimestamp),
			BoostName:         b.name,
			Namespace:         pod.Namespace,
			Pod:               pod.Name,
			Container:         container.Name,
			ContainerCount:    1,
			Scope:             dashboard.EventScopeContainer,
			EventType:         dashboard.EventIncrease,
			CPURequestsBefore: requestBefore,
			CPULimitsBefore:   limitBefore,
			CPURequestsAfter:  cpuResourceString(container.Resources.Requests),
			CPULimitsAfter:    cpuResourceString(container.Resources.Limits),
			...
			BoostTimestamp:    annotation.BoostTimestamp.UTC().Format(time.RFC3339Nano),
			Reason:            "controller observed boosted pod",
		})
	}
	return b.withPodSummaryEvent(pod, annotation, dashboard.EventIncrease, events, "controller observed boosted pod")
}
...
func (b *StartupCPUBoostImpl) revertResources(ctx context.Context, pod *corev1.Pod) error {
	var err error
	log := b.loggerFromContext(ctx)
	events := b.revertEventsFromPod(pod)
...
func (b *StartupCPUBoostImpl) revertEventsFromPod(pod *corev1.Pod) []dashboard.Event {
	annotation, err := bpod.BoostAnnotationFromPod(pod)
	if err != nil {
		return nil
	}
	...
	events := make([]dashboard.Event, 0, len(pod.Spec.Containers)+1)
	for _, container := range pod.Spec.Containers {
		requestAfter, hasRequest := annotation.InitCPURequests[container.Name]
		limitAfter, hasLimit := annotation.InitCPULimits[container.Name]
		if !hasRequest && !hasLimit {
			continue
		}
		ownerKind, ownerName := controllerOwner(pod)
		events = append(events, dashboard.Event{
			EventUID:          eventUID(dashboard.EventDecrease, pod, container.Name, annotation.BoostTimestamp),
			BoostName:         b.name,
			Namespace:         pod.Namespace,
			Pod:               pod.Name,
			Container:         container.Name,
			ContainerCount:    1,
			Scope:             dashboard.EventScopeContainer,
			EventType:         dashboard.EventDecrease,
			CPURequestsBefore: cpuResourceString(container.Resources.Requests),
			CPULimitsBefore:   cpuResourceString(container.Resources.Limits),
			CPURequestsAfter:  requestAfter,
			CPULimitsAfter:    limitAfter,
			PodUID:            string(pod.UID),
			NodeName:          pod.Spec.NodeName,
			OwnerKind:         ownerKind,
			OwnerName:         ownerName,
			...
			Image:             container.Image,
			BoostTimestamp:    annotation.BoostTimestamp.UTC().Format(time.RFC3339Nano),
			Reason:            "duration policy restored original container CPU",
		})
	}
	return b.withPodSummaryEvent(pod, annotation, dashboard.EventDecrease, events, "duration policy restored original pod CPU")
}

func (b *StartupCPUBoostImpl) withPodSummaryEvent(pod *corev1.Pod, annotation *bpod.BoostPodAnnotation,
	eventType string, events []dashboard.Event, reason string) []dashboard.Event {
	if len(events) == 0 {
		return events
	}

	containerNames := make([]string, 0, len(events))
	for _, event := range events {
		if event.Scope == dashboard.EventScopeContainer && event.Container != "" {
			containerNames = append(containerNames, event.Container)
		}
	}
	ownerKind, ownerName := controllerOwner(pod)
	summary := dashboard.Event{
		EventUID:          eventUID(eventType, pod, podEventUIDContainerName, annotation.BoostTimestamp),
		BoostName:         b.name,
		Namespace:         pod.Namespace,
		Pod:               pod.Name,
		Containers:        containerNames,
		ContainerCount:    len(containerNames),
		Scope:             dashboard.EventScopePod,
		EventType:         eventType,
		CPURequestsBefore: sumEventCPUValues(events, func(event dashboard.Event) string { return event.CPURequestsBefore }),
		CPULimitsBefore:   sumEventCPUValues(events, func(event dashboard.Event) string { return event.CPULimitsBefore }),
		CPURequestsAfter:  sumEventCPUValues(events, func(event dashboard.Event) string { return event.CPURequestsAfter }),
		CPULimitsAfter:    sumEventCPUValues(events, func(event dashboard.Event) string { return event.CPULimitsAfter }),
		PodUID:            string(pod.UID),
		NodeName:          pod.Spec.NodeName,
		OwnerKind:         ownerKind,
		OwnerName:         ownerName,
	...
	return append(events, summary)
}

func sumEventCPUValues(events []dashboard.Event, valueFn func(dashboard.Event) string) string {
	total := apiResource.NewMilliQuantity(0, apiResource.DecimalSI)
	hasValue := false
	for _, event := range events {
		value := valueFn(event)
		if value == "" {
...
func cpuResourceString(resources corev1.ResourceList) string {
	if resources == nil {
		return ""
	}
...
func eventUID(eventType string, pod *corev1.Pod, containerName string, boostTimestamp time.Time) string {
	...
	parts := []string{
		...
		podIdentity,
		containerName,
		boostTimestamp.UTC().Format(time.RFC3339Nano),
	}
