...
func (h *podCPUBoostHandler) Handle(ctx context.Context, req admission.Request) admission.Response {
	...
	log := ctrl.LoggerFrom(ctx).WithName("boost-pod-webhook").WithValues(
		"pod", pod.Name,
...
func (h *podCPUBoostHandler) boostContainerResources(ctx context.Context, b boost.StartupCPUBoost,
	...
	boostedContainers := make([]string, 0, len(pod.Spec.Containers))
	for i, container := range pod.Spec.Containers {
		policy, found := b.ResourcePolicy(container.Name)
		if !found {
			continue
		}
		containerLog := log.WithValues(
		...
		if resizeRequiresRestart(container, corev1.ResourceCPU) {
			containerLog.Info("skipping container due to restart policy")
			continue
		}
		if !hasResourcesToIncrease(container) {
			containerLog.Info("skipping container due to lack of resources to increase")
			continue
		}
		resources := policy.NewResources(ctx, &container)
		if h.containerResizeChangesQosClass(originalQosClass, pod, i, resources) {
			containerLog.Info("skipping container due to QOS class change after boost")
			continue
		}
		cpuRequestsAfter := cpuResourceString(resources.Requests)
		cpuLimitsAfter := cpuResourceString(resources.Limits)
		if !resources.Limits.Cpu().IsZero() {
			if h.canRemoveLimit(originalQosClass, containerLog) {
				delete(resources.Limits, corev1.ResourceCPU)
				cpuLimitsAfter = "<removed>"
			}
		}
		...
		containerLog.Info(
			...
			"cpuRequestsAfter", cpuRequestsAfter,
			"cpuLimitsAfter", cpuLimitsAfter,
		)
	}
	if len(annotation.InitCPULimits) > 0 || len(annotation.InitCPURequests) > 0 {
		...
		log.Info(
			...
			"boostedContainerCount", len(boostedContainers),
			"boostedContainers", boostedContainers,
			"qosClass", string(originalQosClass),
		)
	} else {
		log.V(5).Info("pod CPU resources were not increased")
	}
}
